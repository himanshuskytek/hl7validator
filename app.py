...
def load_combined_rules(version, message_type):
    segment_rules = set()
    field_rules = set()
    path = f"hl7apy/v{version.replace('.', '_')}/v{version.replace('.', '_')}.combined.rules"
    if not os.path.exists(path):
        print(f"❌ Combined rules file not found: {path}")
        return segment_rules, field_rules

    print(f"📥 Loading combined rules from {path} for message type: {message_type}")
    with open(path, "r") as f:
        for line in f:
            parts = line.strip().split('"')
            if len(parts) >= 4:
                if line.startswith("SEGMENT_REQUIRED") and parts[1] == message_type:
                    segment_rules.add(parts[3])
                elif line.startswith("FIELD_REQUIRED") and parts[1] == message_type:
                    field_rules.add(parts[3])
    return segment_rules, field_rules
        ...
        if segment == "MSH":
            ...
            for idx in range(1, len(parts)):
                ...
                if val:
                    ...
                    print(f"⚪ {key} = [empty]")
                if field_id == 9 and "^" in val:
                    msg_type_code, trigger_event = val.split("^")
                    field_map["MESSAGE_TYPE"] = f"{msg_type_code}_{trigger_event}"
                elif field_id == 12:
                    field_map["VERSION"] = val
        else:
...
def index():
    ...
    if request.method == "POST":
        ...
        if uploaded_file.filename.endswith((".hl7", ".txt")):
            ...
            if not message_type:
                result = "❌ HL7 message type (MSH.9) not found. Cannot apply message-specific rules."
                return render_template("index.html", result=result)

            segment_rules, field_rules = load_combined_rules(version, message_type)

            # Step 1: Validate required segments
            segment_errors = [seg for seg in segment_rules if seg not in present_segments]
            if segment_errors:
                result += f"❌ Missing required segments for message type {message_type}:\n" + "\n".join(segment_errors) + "\n"
